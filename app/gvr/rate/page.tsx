...
export default function GvrRatePage() {
  ...
  // 2. 선택된 경기의 두 팀 선수들 자동 로드 (players 테이블)
  useEffect(() => {
    if (!activeMatch) return;

    async function loadPlayers() {
      // team_a와 team_b 텍스트와 정확히 일치하는 팀원들만 가져옴
      const { data, error } = await supabase
        .from('players')
        .select('*')
  ...
  const handleSubmit = async () => {
    ...
    if (!error) {
      alert(`${selectedPlayer.name} 선수 평점이 기록되었습니다!`);
      setRating(null);
    } else {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#06101f] px-6 pb-20 pt-48 text-white font-sans">
      ...
      <div className="mx-auto max-w-6xl relative z-10">
        ...
        {/* 🏟️ 경기 선택 탭 (DB matches 테이블 기반 자동 생성) */}
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-10 mb-10 border-b border-white/5">
          {matches.map(m => (
            ...
            >
              ...
              <span className="text-[9px] font-black px-3 py-1 rounded-full bg-cyan-400 text-black w-fit uppercase mt-2">Active</span>
            </button>
          ))}
          {matches.length === 0 && <p className="text-white/20 italic font-black uppercase py-10">No Matches Registered</p>}
        </div>

        <div className="grid gap-16 lg:grid-cols-[1.4fr_0.6fr]">
          {/* 👥 선수 명단 그리드 (경기에 속한 팀들 자동 필터링) */}
            ...
            {selectedPlayer ? (
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="space-y-3">
                   <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">Student ID</p>
                   <input type="text" placeholder="20240001" value={studentId} onChange={(e) => setStudentId(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-sm font-black focus:border-cyan-400 outline-none" />
                </div>
                <div className="grid grid-cols-4 gap-2">
                  {[7, 7.5, 8, 8.5, 9, 9.5, 10].map(v => (
                    <button key={v} onClick={() => setRating(v)} className={`py-4 rounded-xl border font-black italic transition-all ${rating === v ? 'border-cyan-400 bg-cyan-400 text-black shadow-lg' : 'border-white/5 bg-white/5 text-white/30 hover:border-white/20'}`}>{v.toFixed(1)}</button>
                  ))}
                </div>
                <button onClick={handleSubmit} className="w-full py-6 rounded-[2rem] bg-gradient-to-r from-cyan-300 to-lime-300 text-black font-black italic uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all">Submit Rating</button>
              </div>
    ...
    <div className={isAway ? 'text-right' : 'text-left'}>
      ...
      <div className="grid gap-4">
        {players.length > 0 ? players.map((p: any) => (
          <button key={p.id} onClick={() => onSelect(p)} className={`flex items-center gap-6 p-6 rounded-[2.5rem] border transition-all ${isAway ? 'flex-row-reverse' : ''} ${selectedId === p.id ? 'border-cyan-400 bg-cyan-400 text-black shadow-lg' : 'border-white/5 bg-white/[0.02] hover:bg-white/[0.05]'}`}>
